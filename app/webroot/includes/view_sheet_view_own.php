...
if ($character['user_id'] != $userdata['user_id']) {
    SessionHelper::SetFlashMessage('Not allowed to view ' . $character['character_name']);
    Response::Redirect('/');
}
...
if (isset($_POST['submit'])) {
    if (($character_detail['city'] != 'Side Game') &&
        (($character_detail['asst_sanctioned'] == 'Y') || ($character_detail['is_sanctioned']))
    ) {
...
{
  var sURL = "get_sheet.php?action=view_own&character_id=$characterId&character_type="+character_type;
