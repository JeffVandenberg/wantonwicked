	...
	*/
	function session_begin($update_session_page = true)
	{
		...
		if (!empty($this->session_id))
		{
			...
			$temp_user_id = $this->data['user_id']+0;
            $gm_permissions_query = "select * from gm_permissions where id = $temp_user_id;";
            $gm_permissions_result = $db->sql_query($gm_permissions_query);
            $gm_permissions_detail = $db->sql_fetchrow($gm_permissions_result);
            $db->sql_freeresult($gm_permissions_result);
      
    	// Online Game Permissions
			$this->data['userid'] = $temp_user_id;
    	$this->data['is_asst'] = ($gm_permissions_detail['Is_Asst'] == "Y") ? true : false;
    	$this->data['is_gm'] = ($gm_permissions_detail['Is_GM'] == "Y") ? true : false;
    	$this->data['is_head'] = ($gm_permissions_detail['Is_Head'] == "Y") ? true : false;
    	$this->data['is_admin'] = ($gm_permissions_detail['Is_Admin'] == "Y") ? true : false;
    	$this->data['site_id'] = $gm_permissions_detail['Site_ID'];
    	$this->data['cell_id'] = $gm_permissions_detail['Cell_ID'];
    	$this->data['side_game'] = $gm_permissions_detail['Side_Game'];
    	$this->data['wiki_manager'] = $gm_permissions_detail['Wiki_Manager'];
